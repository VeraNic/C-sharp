...
Console.WriteLine(
    "Знакомство с языками программирования. Семинар 8. Дополнительные задачи.\n"
...
int[,] SnakeArray2(int n, int number)
{
    ...
    int row = 0;
    int column = 0; // строка и столбец текущего элемента массива
    while (column < n)
    {
        if (column % 2 == 0)
            for (row = 0; row < n; row++)
                array[row, column] = number++; // чётный столбец, сверху вниз
        else
            for (row = n - 1; row >= 0; row--)
                array[row, column] = number++; // нечётный столбец, снизу вверх
        column++;
    }
    return array;
}
